...
function Annotation({ children, anchor, id, icon, comboClass, distanceFactor, ...props }) {
  return (
      <Html{...props} position={anchor}>
        <div id={id} className={'trigger_popover'}>
          <div className={`trigger_text-wrapper ${comboClass}`}>
            {children}
            {icon && <div className="trigger_icon w-embed">{icon}</div>}
          </div>
        </div>
      </Html>
  )
